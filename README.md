...
- 🎮 **GPU Kullan (aç/kapa)**: Açıkken (varsayılan) ekran kartının **donanım video kodlayıcısı** (NVIDIA NVENC / Intel QuickSync / AMD AMF / Apple VideoToolbox) zorlanır ve her kare bir **WebGL shader geçişinden** geçirilir (keskinleştirme + GPU'nun 3B/işlem motorunu yükler). Kapalıyken yazılım (CPU) kodlayıcı kullanılır. Not: Görev Yöneticisi'nde kodlayıcı yükü **"Video Encode"** motorunda, WebGL geçişi **"3D"** motorunda görünür (varsayılan grafik değil); güçlü kartlar çoğu videoda %100'e ulaşmaz — çözünürlük/fps artırınca GPU yükü artar. Yazılım (donanımsız/SwiftShader) GL algılanırsa geçiş otomatik atlanır (yavaşlatmasın diye).
